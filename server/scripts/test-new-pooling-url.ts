/**
 * 새로운 Connection Pooling URL 테스트
 */
...
async function testNewPoolingUrl() {
  console.log('🔍 새로운 Connection Pooling URL 테스트...\n');
  ...
  try {
    ...
    const client = await Promise.race([
      ...
      new Promise((_, reject) => 
        setTimeout(() => reject(new Error('Connection timeout (20초)')), 20000)
      )
    ]) as any;
    ...
    console.log('\n✅ 모든 테스트 통과! Supabase Connection Pooling 연결이 정상적으로 작동합니다.');
    console.log('\n💡 다음 단계:');
