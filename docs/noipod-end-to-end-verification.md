...
- 앱 백그라운드 진입 (네이티브 셸 한정 — 일반 웹/Replit 미리보기에서는 비활성):
  ...
  **(코드 확인 완료)**
- iOS 백그라운드 BLE 송신 권한: `mobile/app.json` → `ios.infoPlist.UIBackgroundModes`
  에 `bluetooth-central` 선언. 이 권한이 있어야 앱이 백그라운드로 들어간 직후의
  GATT write 가 OS 큐에 보류되지 않고 즉시 송신되어, 위 1·2차 STOP 프레임이
  포그라운드 복귀 전에도 NoiPod 펌웨어에 도달한다. **(설정 확인 완료 — 다음 EAS dev
  build 부터 적용)**
- 안드로이드 백그라운드 BLE 송신 권한: 이미 연결된 GATT 객체에 대한 write 는
  Android 측에서 별도 manifest 권한이나 foreground service 없이도 프로세스가
  살아 있는 동안 동작. `android.permission.BLUETOOTH_CONNECT`(Android 12+) /
  `BLUETOOTH`(Android 11 이하) 가 이미 `mobile/app.json` 의 `android.permissions`
  에 선언되어 있어 추가 변경 불필요. **(코드 확인 완료)**
...
- 백그라운드 진입 시 STOP 자동 송신은 §7.3 코드 경로(웹 visibilitychange + 네이티브
  AppState 안전망)로 보장. iOS는 `mobile/app.json` 의 `ios.infoPlist.UIBackgroundModes`
  에 `bluetooth-central` 을 선언하여, 백그라운드 진입 직후의 GATT write 가 OS 큐에
  보류되지 않고 즉시 송신되도록 권한을 확보함. 안드로이드는 이미 선언된
  `BLUETOOTH_CONNECT` 권한으로 같은 흐름이 동작. 권한 변경은 다음 EAS dev build
  부터 적용되며, 실 하드웨어 검증(§7.3) 시 "백그라운드 1초 이내 LED OFF" 항목으로
  최종 확인한다.
