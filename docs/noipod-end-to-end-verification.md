...
코드 경로: `TrainingSessionPlay`의 `useEffect` cleanup → `engine.destroy()` → `bleWriteControl(CTRL_STOP)`. **(코드 확인 완료)**

| # | 검증 항목 | 기대 동작 | 결과 |
|---|-----------|-----------|------|
| 7.1 | 진행 중 "뒤로"/"취소" 탭 | NoiPod LED 1초 이내 OFF, 점등 중단 | ☐ Pass / ☐ Fail |
| 7.2 | 진행 중 OS 백버튼/제스처로 강제 빠져나감 | 위와 동일하게 STOP 전송 + LED OFF | ☐ Pass / ☐ Fail |
| 7.3 | 진행 중 앱 백그라운드 5초 후 복귀 | 현 빌드 정책상 STOP 자동 송신 미보장 (별도 follow-up 발행됨) | ☐ Observe |
...
- 백그라운드 진입 시 STOP 자동 송신은 미구현 (별도 follow-up 등록됨 — §7.3 참조).
