...
- **접속 플로우**: 첫 접속 시 스플래시 → 3초 후 로그인 (세션마다 `sessionStorage` 기반)
- **자동 리디렉션**: 토큰이 없거나 만료된 경우 스플래시 또는 로그인 페이지로 리디렉션됩니다
...
- ✅ 스플래시 화면 (탭/앱 열 때마다 표시, 3초 후 로그인으로 이동)
...
- **스크롤바 숨김** (MobileLayout, 약관 모달 등 - 스크롤 가능하나 시각적 스크롤바 비표시)
- **홈 화면 배너 자동 슬라이드** (5초 간격, 카운트 표시)
- Pretendard 폰트 사용
- Framer Motion 애니메이션
- **약관 모달 컴포넌트** (재사용 가능한 약관 표시)
- **문의 상세보기 페이지** (sticky 헤더, 뒤로가기·제목 고정)
