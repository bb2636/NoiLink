...
export default function EditProfile() {
  ...
  const isFormValid = () => {
    ...
    const hasNicknameChange = formData.nickname !== user.username;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.nickname !== user.username) {
      ...
      if (response.success) {
        alert('프로필이 수정되었습니다.');
        navigate('/profile');
        // 페이지 새로고침하여 업데이트된 사용자 정보 반영
        window.location.reload();
      } else {
        setError(response.error || '프로필 수정에 실패했습니다.');
      }
    } catch (err) {
    ...
    <div className="min-h-screen" style={{ backgroundColor: '#0A0A0A' }}>
      <div className="max-w-md mx-auto px-4 py-6">
        ...
        <div className="flex items-center mb-6">
          ...
          <h1 className="text-lg font-semibold text-white">프로필 수정</h1>
        </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="px-4 py-3 rounded-lg text-white" style={{ backgroundColor: '#1A1A1A' }}>
                      {user.email || '이메일 없음'}
                    </div>
                  </div>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-400 mb-2">휴대폰 번호</label>
                    <div className="px-4 py-3 rounded-lg text-white" style={{ backgroundColor: '#1A1A1A' }}>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input
                        ...
                        style={{ 
                          backgroundColor: '#373C39', 
                          borderColor: errors.confirmPassword ? '#ef4444' : '#4B5563' 
                        }}
                        autoComplete="off"
                      />
                      <button
                ...
                <button
                  ...
                  className="w-full py-3 px-4 rounded-lg font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  style={{
                    backgroundColor: isFormValid() ? '#AAED10' : '#373C39',
                    color: isFormValid() ? '#000000' : '#B6B6B9'
                  }}
                >
                  {loading ? '수정 중...' : '수정하기'}
                </button>
