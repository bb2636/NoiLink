...
// (Task #114 이후 직전 점수는 세션 단건 엔드포인트로 조회하므로 userId 불필요.)
...
describe('Result — 결과 재진입 시 서버에서 회복 타임라인 다시 불러오기 (Task #75)', () => {
  ...
  it('state.replayed 가 true 면 "이미 저장된 결과를 불러왔어요" 힌트가 노출된다', () => {
    ...
    renderResult({ replayed: true, recoverySegments: [] });
  ...
  it('state.replayed 가 없거나 false 면 힌트는 그려지지 않는다 (정상 첫 저장 회귀 보호)', () => {
    renderResult({ recoverySegments: [] });
    expect(container?.querySelector('[data-testid="replayed-hint"]')).toBeNull();
    unmountResult();

    renderResult({ replayed: false, recoverySegments: [] });
    expect(container?.querySelector('[data-testid="replayed-hint"]')).toBeNull();
  });
...
//   3. 재진입에서 직전 점수는 세션 단건 엔드포인트
//      (`/metrics/session/:sessionId/previous-score`) 응답의 previousScore 를
//      그대로 채택해 비교 카드와 코칭 메시지에 사용한다 (Task #114).
...
describe('Result — 재진입 시 점수도 서버에서 다시 불러오기 (Task #95)', () => {
  ...
  it('재진입 시 score 6대 지표 평균이 점수 원에 노출된다 (데모 폴백 금지)', async () => {
    ...
    // /metrics/session/:id/previous-score 응답: 직전 세션 점수 58
    mockApiGet.mockImplementation((url: string) => {
      if (url.endsWith('/previous-score')) {
        return Promise.resolve({
          success: true,
          data: { previousScore: 58 },
        });
      }
    ...
    expect(mockApiGet).toHaveBeenCalledWith(
      '/metrics/session/sess-test/previous-score',
    );
  ...
  it('재진입에서 직전 세션 이력이 없으면 비교 카드와 "직전 대비" 코칭 문구가 숨겨진다', async () => {
    mockApiGet.mockImplementation((url: string) => {
      if (url.endsWith('/previous-score')) {
        // 첫 세션 — 서버는 previousScore: null 로 응답한다.
        return Promise.resolve({
          success: true,
  ...
  it('정상 완료 흐름(state.displayScore 존재)은 추가 네트워크 호출 없이 그대로 동작한다', async () => {
    ...
    const previousScoreCalls = mockApiGet.mock.calls.filter(
      (call: unknown[]) =>
        typeof call[0] === 'string' && call[0].endsWith('/previous-score'),
    );
