...
// Task #122 이후 TrainingSessionPlay 자체는 직전 점수 조회를 직접 호출하지 않고
// (submit 유틸이 결과 객체에 함께 담아 돌려준다) `api` 모듈을 import 도 하지 않는다.
// 다만 다른 자식 컴포넌트나 향후 변경에서 우연히 import 되더라도 jsdom 환경의
// 네트워크 부수효과가 새지 않도록 가벼운 빈 응답 mock 만 그대로 유지한다.
// `/sessions/user/...` 로의 회귀를 막는 회귀 테스트도 이 mock 의 `apiGet.mock.calls`
// 를 들여다보고 그 경로로의 호출이 비어 있는지 직접 검증한다.
vi.mock('../utils/api', () => ({
  __esModule: true,
...
describe('TrainingSessionPlay — 백그라운드 진입/부분 결과 저장 분기', () => {
  ...
  // ─────────────────────────────────────────────────────────
  // Task #113 / Task #122 — 결과로 navigate 할 때 직전 점수를 함께 실어 보낸다
  // ─────────────────────────────────────────────────────────
  // 정책 요약 (Task #122 이후):
  //   1. runSubmit 은 submitCompletedTrainingWithRetry 에 `includePreviousScore: true`
  //      를 넘긴다. submit 유틸이 calculateMetrics 와 같은 단건 엔드포인트
  //      (`/metrics/session/:id/previous-score`) 를 병렬로 호출해 결과 객체에
  //      previousScore/previousScoreCreatedAt 를 함께 담아 돌려준다.
  //   2. TrainingSessionPlay 는 그 값을 그대로 navigate state 로 흘려보낸다 —
  //      별도의 사용자 이력(`/sessions/user/:userId`) 호출은 하지 않는다.
  //   3. submit 결과의 previousScore 가 null/undefined 면 navigate state 도
  //      undefined 로 변환돼 Result.tsx 가 비교 카드를 자연스럽게 숨긴다.
  // 보호 목적:
  //   - 정상 완료 직후에도 가짜 폴백(`todayScore - 12`) 이 다시 새어 나오지 않도록
  //     navigate state.previousScore 가 submit 결과로 채워지는 경로를 잠근다.
  //   - 정상 완료 흐름이 다시 페이징 이력 호출(`/sessions/user/...`) 로 회귀하지
  //     않도록 그 경로가 사용되지 않음을 확인한다 — Task #122 의 통합 정책 보호.

  it('runSubmit 후 navigate state.previousScore/previousScoreCreatedAt/previousScoreLocalDate 가 submit 결과의 값으로 채워진다 (Task #122 / Task #123 / Task #132)', async () => {
    // submit 유틸이 단건 엔드포인트로 직전 점수와 KST 표시용 날짜까지 한 번에
    // 받아 돌려주는 흐름을 모킹. Task #122 가 정상 완료 흐름을 단일 엔드포인트로
    // 통합하면서, Task #132 의 KST 표시용 날짜도 같은 응답으로 전달된다.
    const { submitCompletedTrainingWithRetry } = await import('../utils/submitTrainingRun');
    const submitMock = submitCompletedTrainingWithRetry as unknown as ReturnType<typeof vi.fn>;
    ...
    // 사용자 이력 호출(`/sessions/user/...`) 은 더 이상 일어나지 않아야 한다 —
    // 그 경로로의 회귀를 막기 위해 호출 자체가 비어 있는지 확인한다.
    ...
    await act(async () => {
      confirmBtn!.click();
      // submit + navigate 까지 microtask 큐를 충분히 비운다.
      await Promise.resolve();
      await Promise.resolve();
      await Promise.resolve();
      await Promise.resolve();
    ...
    // submit 호출에 includePreviousScore=true 가 흘러갔어야 한다 — 정상 완료
    // 흐름에서 직전 점수까지 한 번에 받는 정책의 신호.
    expect(submitMock).toHaveBeenCalledTimes(1);
    expect(submitMock.mock.calls[0][0]).toMatchObject({ includePreviousScore: true });

    // 더 이상 페이징 이력 엔드포인트(`/sessions/user/:userId`) 로의 호출은 없다 —
    // Task #122 가 정상 완료 흐름의 직전 점수 조회를 단건 엔드포인트로 통합한 결과.
    const userHistoryCalls = apiGet.mock.calls.filter(
      (call) =>
        typeof call[0] === 'string' && (call[0] as string).startsWith('/sessions/user/'),
    );
    expect(userHistoryCalls.length).toBe(0);

    // navigate state.previousScore 가 submit 결과의 73 으로 채워져야 한다.
    // 비교 카드 라벨용 createdAt 도 같은 세션의 값으로 한 쌍으로 함께 전달돼야 한다 —
    // 가짜 "오늘 - 2일" 라벨이 다시 새어 나오지 않게 잠근다.
  ...
  it('자정 경계: 직전 세션이 UTC 15:00 직전이면 previousScoreLocalDate 는 KST 의 다음 날짜로 떨어진다 (Task #132)', async () => {
    ...
    // ISO 만 보면 디바이스 로컬 시간대로 라벨이 흔들릴 수 있지만, 서버가 같은
    // KST 헬퍼(`shared/kst-date.ts` 의 `isoToKstLocalDate`) 로 계산해 보내는
    // 표시용 날짜는 항상 "2026-04-25" 이어야 한다 — submit 유틸이 이 값을
    // 그대로 흘려보내고, TrainingSessionPlay 가 navigate state 로 전달한다.
    const { submitCompletedTrainingWithRetry } = await import('../utils/submitTrainingRun');
    const submitMock = submitCompletedTrainingWithRetry as unknown as ReturnType<typeof vi.fn>;
    submitMock.mockResolvedValueOnce({
      error: null,
      ...
      // 서버 단건 엔드포인트가 KST 헬퍼로 만들어 보내는 값 — 자정 직후로 떨어진다.
      previousScoreLocalDate: '2026-04-25',
    });
  ...
  it('submit 결과의 previousScore 가 null 이면 navigate state 도 undefined 로 변환된다 (첫 세션 정책)', async () => {
    const { submitCompletedTrainingWithRetry } = await import('../utils/submitTrainingRun');
    const submitMock = submitCompletedTrainingWithRetry as unknown as ReturnType<typeof vi.fn>;
    // 첫 세션 또는 직전 세션 점수 없음 — submit 유틸이 null/null 한 쌍으로 정규화.
    submitMock.mockResolvedValueOnce({
      error: null,
      displayScore: 60,
      sessionId: 'test-session',
  ...
  it('submit 결과에 previousScore 필드가 누락돼도 navigate 는 진행되고 비교 카드는 숨김 처리된다 (이전 응답 형식 호환)', async () => {
    const { submitCompletedTrainingWithRetry } = await import('../utils/submitTrainingRun');
    const submitMock = submitCompletedTrainingWithRetry as unknown as ReturnType<typeof vi.fn>;
    submitMock.mockResolvedValueOnce({
      error: null,
      ...
      sessionCreated: true,
      totalAttempts: 1,
      // previousScore / previousScoreCreatedAt 가 아예 없음 — 옛 응답 형식 호환.
    });
